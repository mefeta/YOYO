...
// Reports requires admin or manager role
...
router.get('/sla', (req, res) => {
  try {
    ...
    const breached = allRequests.filter(r => {
      if (!r.slaDeadline || ['resolved', 'closed'].includes(r.status as string)) return false;
      return new Date(r.slaDeadline).getTime() < now;
    });
    const compliant = allRequests.filter(r => {
      if (!r.slaDeadline) return false; // Don't count requests without SLA as compliant
      return ['resolved', 'closed'].includes(r.status as string) || new Date(r.slaDeadline).getTime() >= now;
    });
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayBreached = dayReqs.filter(r => r.slaDeadline && new Date(r.slaDeadline).getTime() < now && !['resolved', 'closed'].includes(r.status as string));
      daily.push({
        date: day,
  ...
  try {
    ...
    switch (type) {
      case 'operational_performance':
        const open = allRequests.filter(r => !['resolved', 'closed'].includes(r.status as string)).length;
    ...
    db.insert(schema.reports).values({
      ...
      generatedBy: req.headers['x-user-id'] as string || null,
      summary,
      createdAt: now,
    }).run();
