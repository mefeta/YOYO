...
async function main() {
    ...
    const totalLines = 200000; // Known: 200K data rows
  ...
  `);
    ...
    for (const u of users) {
        insertUser.run(u.id, u.name, u.email, Buffer.from(JSON.stringify({ email: u.email, password: u.password })).toString('base64'), u.role, new Date().toISOString());
    }
    ...
    for (const a of agentData) {
        ...
        agents.push({ id, name: a.name, teamId: team?.id || teams[0].id, sectors: a.sectors });
        insertAgent.run(id, null, team?.id || teams[0].id, 'Support Agent', a.skills, a.sectors, '["Turkish","English"]', 'available', 15, 0, 0, 0, new Date().toISOString());
    }
    ...
    let requestCount = 0;
    let skippedLines = 0;
    const insertCustomer = sqlite.prepare('INSERT OR IGNORE INTO customers (id, name, email, company, tier, sector, consent_status, age, gender, tenure_months, previous_tickets, satisfaction_score, segment, region, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
  ...
  `);
    ...
    function flushBatches() {
        const insertCustomers = sqlite.transaction((batch) => {
            for (const row of batch)
                insertCustomer.run(...row);
        });
        const insertRequests = sqlite.transaction((batch) => {
            for (const row of batch)
                insertRequest.run(...row);
        });
        const insertEvents = sqlite.transaction((batch) => {
            for (const row of batch)
                insertEvent.run(...row);
        });
    ...
    const insertInt = sqlite.prepare('INSERT INTO integrations (id, name, type, status, last_sync_at, config, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)');
    const integrations = [
        { name: 'Email Gateway', type: 'email', status: 'connected' },
        { name: 'Web Portal', type: 'portal', status: 'connected' },
        { name: 'Slack Channel', type: 'slack', status: 'disconnected' },
        { name: 'API Gateway', type: 'api', status: 'connected' },
        { name: 'Data Warehouse', type: 'data_warehouse', status: 'connected' },
    ];
    for (const int of integrations) {
        insertInt.run(uuid(), int.name, int.type, int.status, new Date().toISOString(), '{}', now);
    }
    // ===== CREATE REPORTS =====
    ...
    const insertAudit = sqlite.prepare('INSERT INTO audit_logs (id, actor_id, action, entity_type, entity_id, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)');
    ...
    sqlite.exec('ANALYZE;');
    // Count results
    const countCustomers = sqlite.prepare('SELECT COUNT(*) as c FROM customers').get().c;
    const countRequests = sqlite.prepare('SELECT COUNT(*) as c FROM requests').get().c;
    const countAgents = sqlite.prepare('SELECT COUNT(*) as c FROM agents').get().c;
    sqlite.close();
    console.log('\n========================================');
