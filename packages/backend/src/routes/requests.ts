...
// Recalculate an agent's workload from actual DB counts
function syncAgentWorkload(agentId: string) {
  const count = (db.select({ count: sql<number>`COUNT(*)` }).from(schema.requests)
    .where(and(
      eq(schema.requests.assignedAgentId, agentId),
      sql`status NOT IN ('resolved', 'closed')`
    )).get() as any)?.count || 0;
  db.update(schema.agents).set({ currentWorkload: Number(count) }).where(eq(schema.agents.id, agentId)).run();
}
...
router.get('/', (req, res) => {
  try {
    ...
    const limitVal = Math.min(parseInt(limit as string) || 100, 500);
    ...
    const totalCount = conditions.length > 0
      ? (db.select({ count: sql`COUNT(*)` }).from(schema.requests).where(and(...conditions)).get() as any)?.count || 0
      : (db.select({ count: sql`COUNT(*)` }).from(schema.requests).get() as any)?.count || 0;
...
router.post('/', validate(createRequestSchema), (req, res) => {
  try {
    const { title, description, customerId, customerName, customerCompany, sector, channel, category } = req.body;

    const id = `REQ-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
    const now = new Date().toISOString();

    // Create customer record if name provided but no customerId
...
router.post('/:id/assign', validate(assignSchema), (req, res) => {
  try {
    ...
    const slaMinutes: Record<string, number> = { critical: 30, high: 120, medium: 480, low: 1440 };
    ...
    // Sync agent workload from DB
    if (agentId) {
      syncAgentWorkload(agentId);
    }
...
router.patch('/:id', (req, res) => {
  try {
    ...
    if (updates.status && updates.status !== r.status) {
      ...
      // Track resolvedTicketsCount on status change
      const wasResolvedOrClosed = r.status === 'resolved' || r.status === 'closed';
      const isNowResolvedOrClosed = updates.status === 'resolved' || updates.status === 'closed';
      const agentId = r.assignedAgentId || updates.assignedAgentId;
      if (agentId) {
        if (isNowResolvedOrClosed && !wasResolvedOrClosed) {
          db.update(schema.agents).set({
            resolvedTicketsCount: sql`COALESCE(resolved_tickets_count, 0) + 1`,
          }).where(eq(schema.agents.id, agentId)).run();
        } else if (wasResolvedOrClosed && !isNowResolvedOrClosed) {
          updates.resolvedAt = null;
          db.update(schema.agents).set({
            resolvedTicketsCount: sql`MAX(COALESCE(resolved_tickets_count, 1) - 1, 0)`,
          }).where(eq(schema.agents.id, agentId)).run();
        }
      }
    }

    // Sync workload if agent assignment changed or status changed
    if (updates.assignedAgentId || (updates.status && ['resolved', 'closed'].includes(updates.status))) {
      if (r.assignedAgentId) syncAgentWorkload(r.assignedAgentId);
      if (updates.assignedAgentId && updates.assignedAgentId !== r.assignedAgentId) {
        syncAgentWorkload(updates.assignedAgentId as string);
      }
    }
...
router.post('/:id/status', validate(statusSchema), (req, res) => {
  try {
    ...
    const wasResolvedOrClosed = r.status === 'resolved' || r.status === 'closed';
    const isNowResolvedOrClosed = status === 'resolved' || status === 'closed';

    if (isNowResolvedOrClosed) {
      updates.resolvedAt = now;
      if (r.assignedAgentId) {
        syncAgentWorkload(r.assignedAgentId);
        // Increment resolvedTicketsCount
        db.update(schema.agents).set({
          resolvedTicketsCount: sql`COALESCE(resolved_tickets_count, 0) + 1`,
        }).where(eq(schema.agents.id, r.assignedAgentId)).run();
      }
    } else if (wasResolvedOrClosed && !isNowResolvedOrClosed) {
      // Reopened — clear resolvedAt and sync workload back up
      updates.resolvedAt = null;
      if (r.assignedAgentId) {
        syncAgentWorkload(r.assignedAgentId);
        // Decrement resolvedTicketsCount
        db.update(schema.agents).set({
          resolvedTicketsCount: sql`MAX(COALESCE(resolved_tickets_count, 1) - 1, 0)`,
        }).where(eq(schema.agents.id, r.assignedAgentId)).run();
      }
...
router.get('/:id/comments', (req, res) => {
  try {
    const comments = db.select().from(schema.requestComments)
      .where(eq(schema.requestComments.requestId, String(req.params.id)))
      .orderBy(desc(schema.requestComments.createdAt))
      .all();
    res.json(comments);
  } catch (error) {
...
router.post('/:id/comments', (req: AuthRequest, res) => {
  try {
    ...
    db.insert(schema.requestComments).values({
      id,
      requestId: r.id,
    ...
    const created = db.select().from(schema.requestComments).where(eq(schema.requestComments.id, id)).get();
    res.status(201).json(created);
  } catch (error) {
