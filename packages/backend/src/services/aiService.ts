...
import type { AIAnalysisResult, AgentScore, AssignmentRecommendation } from '../types/index.js';

const SKILL_KEYWORDS: Record<string, string[]> = {
  'Delivery Delay': ['delivery', 'shipping', 'shipment', 'customs', 'address', 'delay', 'parcel', 'logistics', 'transport'],
  'Billing Problem': ['invoice', 'charge', 'payment', 'bank', 'accounting', 'tax', 'declaration', 'amount', 'transfer'],
  'Technical Failure': ['server', 'pbx', 'mobile', 'activation', 'network', 'fiber', 'system', 'performance', 'database', 'maintenance', 'hardware', 'device', 'stock', 'count', 'data'],
  'Campaign Approval': ['campaign', 'approval', 'budget', 'launch', 'brief', 'creative', 'advertisement'],
  'Customer Complaint': ['complaint', 'dissatisfied', 'crisis', 'socialmedia', 'customer', 'victim'],
  'Contract Update': ['contract', 'agreement', 'supplier', 'price', 'number', 'porting', 'store', 'opening'],
  'Outage Report': ['outage', 'fiber', 'downtown', 'failure'],
  'Data Request': ['report', 'analysis', 'seo', 'survey', 'sla', 'credit', 'assessment'],
  'Refund Request': ['refund', 'return'],
  'Compliance Question': ['gdpr', 'data', 'deletion', 'legal', 'tax', 'declaration', 'consulting', 'compliance'],
};

const SECTOR_KEYWORDS: Record<string, string[]> = {
  Logistics: ['cargo', 'logistics', 'delivery', 'shipment', 'parcel', 'warehouse', 'transport', 'customs', 'address'],
  Telecom: ['fiber', 'mobile', 'line', 'pbx', 'internet', 'telecom', 'operator', 'voip'],
  Marketing: ['campaign', 'advertisement', 'socialmedia', 'seo', 'brand', 'marketing', 'creative', 'launch', 'brief'],
  Finance: ['invoice', 'payment', 'bank', 'tax', 'accounting', 'credit', 'declaration', 'transfer', 'amount'],
  Retail: ['store', 'stock', 'return', 'retail', 'supplier', 'product'],
  Healthcare: ['patient', 'appointment', 'medical', 'database', 'hardware', 'maintenance', 'health', 'survey'],
};
...
function detectSentiment(text: string): { sentiment: 'positive' | 'neutral' | 'negative' | 'angry'; score: number } {
  const lower = text.toLowerCase();
  const negativeWords = ['complaint', 'issue', 'delay', 'victim', 'failed', 'outage', 'threat', 'termination', 'crisis', 'negative', 'error', 'missing', 'bad', 'terrible', 'disaster'];
  const angryWords = ['threat', 'termination', 'crisis', 'disaster', 'terrible', 'angry', 'furious', 'lawsuit'];
  const positiveWords = ['thanks', 'satisfied', 'positive', 'successful', 'great', 'good', 'opportunity', 'excellent'];
  ...
  if (angryScore > 0) return { sentiment: 'angry', score: 10 + angryScore * 20 };
  if (negScore > 2) return { sentiment: 'negative', score: 5 + negScore * 15 };
  if (posScore > 2) return { sentiment: 'positive', score: 10 + posScore * 10 };
  if (negScore > 0) return { sentiment: 'negative', score: negScore * 10 };
  if (posScore > 0) return { sentiment: 'positive', score: posScore * 5 };
  return { sentiment: 'neutral', score: 50 };
}

function detectUrgency(title: string, description: string): number {
  const fullText = `${title} ${description}`.toLowerCase();
  const urgentWords = ['urgent', 'immediately', 'critical', 'important', 'threat', 'termination', 'outage', 'crisis', 'deadline', 'asap', 'time'];
  const mediumWords = ['delay', 'issue', 'help', 'support', 'problem', 'request'];

  let urgency = 50;
  urgentWords.forEach(w => {
    if (fullText.includes(w)) urgency += 15;
  });
  mediumWords.forEach(w => {
    if (fullText.includes(w)) urgency += 5;
  });
  return Math.min(urgency, 100);
}

function detectPriority(urgency: number, sentiment: string): 'critical' | 'high' | 'medium' | 'low' {
  if (urgency >= 80 || sentiment === 'angry') return 'critical';
  if (urgency >= 60) return 'high';
  if (urgency >= 40) return 'medium';
  return 'low';
}

function calculateSLARisk(priority: string, urgency: number): number {
  const base = priority === 'critical' ? 80 : priority === 'high' ? 60 : priority === 'medium' ? 30 : 10;
  return Math.min(base + urgency * 0.2, 100);
}

function estimateResolutionTime(category: string, priority: string): number {
...
function generateSummary(category: string, sentiment: string, priority: string): string {
...
function generateExplanation(category: string, priority: string, sentiment: string, urgency: number, tags: string[]): string {
  const parts: string[] = [];
  parts.push(`Request classified as "${category}"`);
  parts.push(`priority level: ${priority}`);
  if (sentiment !== 'neutral') parts.push(`customer sentiment: ${sentiment}`);
  if (urgency > 70) parts.push(`high urgency score (${Math.round(urgency)}/100)`);
  if (tags.length > 0) parts.push(`keywords: ${tags.slice(0, 3).join(', ')}`);
  return parts.join('. ') + '.';
}

function calculateSkillMatch(agent: any, category: string, sector: string): number {
  const agentSkills: string[] = JSON.parse(agent.skills || '[]');
  const agentSectors: string[] = JSON.parse(agent.sectors || '[]');
  ...
  for (const sk of agentSkills) {
    if (catLower.includes(sk.toLowerCase().split(' ')[0])) skillScore += 25;
  }

  let sectorScore = 0;
  for (const sec of agentSectors) {
    if (sec.toLowerCase() === sectorLower) sectorScore += 40;
    if (sectorLower.includes(sec.toLowerCase())) sectorScore += 20;
  }

  return Math.min(skillScore + sectorScore + 20, 100);
}

function calculateWorkloadScore(agent: any): number {
...
function calculateAvailabilityScore(agent: any): number {
  const status = agent.availabilityStatus || 'offline';
  const scores: Record<string, number> = { available: 100, busy: 40, away: 20, offline: 0 };
  return scores[status] || 0;
}

function calculateHistoricalScore(agent: any): number {
...
function calculateSLAFitScore(agent: any, priority: string): number {
  const avgTime = agent.avgResolutionTime || 5;
  const target = priority === 'critical' ? 1 : priority === 'high' ? 3 : 5;
  if (avgTime <= target) return 100;
...
export function analyzeRequest(request: {
  ...
  if (slaRisk > 70) riskFlags.push('High SLA risk');
  if (sentiment === 'angry') riskFlags.push('Customer angry - priority intervention');
  if (urgency > 80) riskFlags.push('Critical urgency level');
  if (sentiment === 'negative') riskFlags.push('Negative customer experience risk');
...
export function recommendAgent(
  request: { id: string; title: string; description: string; sector: string; category: string; priority: string },
  agentsList: any[]
  ...
  const scoredAgents: AgentScore[] = agentsList.map((agent) => {
    ...
    if (availability > 60) parts.push('currently available');
...
export function generateInsights(requests: any[], agents: any[]): string[] {
  ...
  const slaRisk: Record<string, number> = {};
  for (const r of requests) {
